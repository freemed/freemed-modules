...
class MultipleProcedureModule extends EMRModule {
	...
	var $MODULE_NAME = "Procedures (Bulk)";
	...
	function MultipleProcedureModule () {
		...
		$this->EMRModule();
	} // end constructor MultipleProcedureModule
	...
	function addform() {
		...
		for ($i=1; $i<=6; $i++) {
			$grid .= "<tr>\n".
				"<td>".module_function('cptmodule', 'widget', array ( 'proccpt['.$i.']' ))."</td>\n".
				)."</td>\n".
		...
		for ($i=1; $i<=6; $i++) {
			...
			if ($proccpt[$i]) {
				...
				$charge[$i] = $this->CalculateCharge($proccov1,$proccpt[$i],$procphysician,$patient);
				$cpt_code[$i] = freemed::get_link_rec ($proccpt[$i], "cpt"); // cpt code
				// Interface
				$confirmgrid .= "<tr>\n".
		...
		{
			//print "<br/><br/><hr/>\n";
			//print "__action = ".$_REQUEST['__action']."<br/>\n";
			//print "proccpt = "; print_r($proccpt); print "<br/>\n";
			//print "procbalorig = "; print_r($procbalorig); print "<br/>\n";
			// display the wizard
		...
		if ($wizard->is_done())
		{
			...
			//print "<br/><br/><hr/>\n";
			//print "proccpt = ".print_r($proccpt)."<br/>\n";
			//print "procbalorig = ".print_r($procbalorig)."<br/>\n";
			for ($i=1; $i<=6; $i++) {
				if ($proccpt[$i]) {
	...
	} // end function MultipleProcedureModule->view()
	...
	} // end function MultipleProcedureModule->GetAuthorizations()
	...
	function CalculateCharge($covid,$cptid,$phyid,$patid)  {
		...
		return $charge;
	} // end function MultipleProcedureModule->CalculateCharge()
